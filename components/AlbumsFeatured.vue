...
<template>
  ...
  <div v-for="album in featuredAlbums" :key="album.id" class="mb-6">
    <div class="flex items-start">
      <img :src="useAssets(album.image)" :alt="album.title">
      <div class="ml-6 leading-4 description flex flex-col justify-between ">
        {{ album.description }}
        <div class="flex mt-6">
          <NuxtLink :to="`/products/${album.slug}`" :class="album.quantity > 0 ? 'bg-black' : 'bg-gray-700'" class=" px-8 py-2 text-3xl  font-bold text-white rounded-none flex items-center justify-center">
            <span v-if="album.quantity > 0">order</span>
