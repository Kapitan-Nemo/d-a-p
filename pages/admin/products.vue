    ...
    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
      ...
      <tbody>
        <tr v-for="product in albums" :key="product.id">
          ...
          <td class="px-6 py-4">
            <button class="mr-6" @click="editMode = !editMode">
          ...
          <div id="drawer-example" :class="editMode ? 'transform-none' : '-translate-x-full' " class="fixed top-0 left-0 z-40 h-screen p-4 overflow-y-auto transition-transform bg-white w-80 dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-label">
            ...
            <div class="flex flex-col">
              <input v-model="product.description" class="mb-3" type="text">
              <input v-model="product.title" class="mb-3" type="text">
              <input v-model="product.price" class="mb-3" type="text">
            </div>
          </div>
        </tr>
      </tbody>
